    ...
    public static class PlayerControlFixedUpdatePatch {
        ...
        static void setBasePlayerOutlines() {
            foreach (PlayerControl target in CachedPlayer.AllPlayers) {
                ...
                if (Camouflager.camouflageTimer <= 0f && !Helpers.MushroomSabotageActive() && MapOptionsTor.firstKillPlayer != null && MapOptionsTor.shieldFirstKill && ((target == MapOptionsTor.firstKillPlayer && !isMorphedMorphling) || (isMorphedMorphling && Morphling.morphTarget == MapOptionsTor.firstKillPlayer))) {
                    hasVisibleShield = true;
                    color = Color.blue;
                }
        ...
        static void detectiveUpdateFootPrints() {
            ...
            if (Detective.timer <= 0f) {
                ...
                foreach (PlayerControl player in CachedPlayer.AllPlayers) {
                    if (player != null && player != CachedPlayer.LocalPlayer.PlayerControl && !player.Data.IsDead && !player.inVent && !player == Ninja.ninja && Ninja.stealthed) 
                    {
