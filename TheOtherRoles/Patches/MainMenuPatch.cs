    ...
    public class MainMenuPatch
    {
        ...
        private static void Prefix(MainMenuManager __instance)
        {
            ...
            if (Helpers.isChinese())
            {
                ...
                {
                    var fangkuaiAUQQObj = Object.Instantiate(template, null);
                    Object.Destroy(fangkuaiAUQQObj.GetComponent<AspectPosition>());
                    fangkuaiAUQQObj.transform.localPosition = new(1.4764f, -2.2764f, 0);
                    var fangkuaiAUQQbutton = fangkuaiAUQQObj.GetComponentInChildren<TextMeshPro>();
                    fangkuaiAUQQbutton.transform.localPosition = new(1.4764f, -2.2764f, 0);
                    fangkuaiAUQQbutton.alignment = TextAlignmentOptions.Right;
                    __instance.StartCoroutine(Effects.Lerp(0.1f, new System.Action<float>((p) =>
                    {
                        fangkuaiAUQQbutton.SetText(Helpers.transl("ChineseQQqunfangkuai"));
                    })));

                    PassiveButton passiveButtonxiaozhan = fangkuaiAUQQObj.GetComponent<PassiveButton>();
                    SpriteRenderer buttonSpritefangkuaiAUQQ = fangkuaiAUQQObj.transform.FindChild("Inactive").GetComponent<SpriteRenderer>();

                    passiveButtonxiaozhan.OnClick = new Button.ButtonClickedEvent();
                    passiveButtonxiaozhan.OnClick.AddListener((System.Action)(() => Application.OpenURL(Helpers.transl("ChineseQQqunfangkuaiURL"))));
                    Color fangkuaiAUQQColor = new Color32(255, 0, 0, byte.MaxValue);
                    buttonSpritefangkuaiAUQQ.color = fangkuaiAUQQbutton.color = fangkuaiAUQQColor;
            ...
            buttonDiscord.transform.localPosition = new(4.459f, -1.5f, 0);
...
All-The-Roles - Some button photo</size>";
                creditsString += "</align>";
    ...
    public class VersionShower_Start
    {
        public static void Postfix(VersionShower __instance)
        {
            __instance.text.text = $"Among Us{Application.version}-<color=#ff351f>The Other Roles Community Edition</color> <color=#FCCE03FF>v{TheOtherRolesPlugin.Version.ToString() + (TheOtherRolesPlugin.betaDays > 0 ? "-BETA" : "")}</color>";
        }
    }
}
