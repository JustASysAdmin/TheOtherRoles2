...
namespace TheOtherRoles.Patches {
    ...
    public class OnGameEndPatch {
        ...
        public static void Postfix(AmongUsClient __instance, [HarmonyArgument(0)]ref EndGameResult endGameResult) {
            ...
            // Remove Swopper, Jester, Arsonist, Vulture, Jackal, former Jackals and Sidekick from winners (if they win, they'll be readded)
                ...
                if (!Lovers.existingWithKiller()) {
                    ...
                    foreach (PlayerControl p in CachedPlayer.AllPlayers) {
                        ...
                        if (p == Lovers.lover1 || p == Lovers.lover2)
                            ...
                            TempData.winners.Add(new WinningPlayerData(p.Data));
                        else if (p != Jester.jester && p != Swooper.swooper && p != Jackal.jackal && p != Sidekick.sidekick && p != Arsonist.arsonist && p != Vulture.vulture && !Jackal.formerJackals.Contains(p) && !p.Data.Role.IsImpostor)
                            TempData.winners.Add(new WinningPlayerData(p.Data));
                    }
                }
                // Lovers solo win
    ...
    class CheckEndCriteriaPatch {
        ...
        private static bool CheckAndEndGameForSwooperWin(ShipStatus __instance, PlayerStatistics statistics)
        {
            ...
            if (statistics.TeamSwooperAlive >= statistics.TotalAlive - statistics.TeamSwooperAlive && statistics.TeamImpostorsAlive == 0 && statistics.TeamJackalAlive == 0 && !(statistics.TeamSwooperHasAliveLover && statistics.TeamLoversAlive == 2))
            {
                //__instance.enabled = false;
        ...
        private static bool CheckAndEndGameForImpostorWin(ShipStatus __instance, PlayerStatistics statistics) {
            ...
            if (statistics.TeamImpostorsAlive >= statistics.TotalAlive - statistics.TeamImpostorsAlive && statistics.TeamSwooperAlive == 0 && statistics.TeamJackalAlive == 0 && !(statistics.TeamImpostorHasAliveLover && statistics.TeamLoversAlive == 2)) {
                //__instance.enabled = false;
