...
namespace TheOtherRoles.Patches {
    ...
    class HudManagerUpdatePatch
    {
        ...
        static void mrFreezeFreezeAndUnfreeze()
        {
            if (MrFreeze.mrFreeze != null)
            {
                ...
                if (MrFreeze.mrFreezeTimer > 0)
                {
                    ...
                    {
                        if (p != MrFreeze.mrFreeze && !p.Data.Role.IsImpostor)
                        {
                            p.MyPhysics.Speed = 0;
                        }
                    }
        ...
        public static void miniUpdate() {
            if (Mini.mini == null || Camouflager.camouflageTimer > 0f || Helpers.MushroomSabotageActive() || Mini.mini == Morphling.morphling && Morphling.morphTimer > 0f || Mini.mini == Ninja.ninja && Ninja.stealthed || SurveillanceMinigamePatch.nightVisionIsActive) return;
        ...
        {
            if (MeetingHud.Instance || MapOptionsTor.gameMode == CustomGamemodes.HideNSeek || MapOptionsTor.gameMode == CustomGamemodes.PropHunt) __instance.SabotageButton.Hide();
