...
namespace TheOtherRoles
{
    public class CustomOption
    {
        ...
        public CustomOption(int id, CustomOptionType type, string name, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, Action onChange = null)
        {
            ...
            if (id != 0)
            {
                entry = TheOtherRolesPlugin.Instance.Config.Bind(string.Format(Helpers.transl("PresetCustomOptions"), preset), id.ToString(), defaultSelection);
        ...
        {
            return new CustomOption(id, type, name, new[] { "optionOff".Translate(), "optionOn".Translate() }, defaultValue ? "optionOn".Translate() : "optionOff".Translate(), parent, isHeader,onChange);
        }
        ...
        public static void switchPreset(int newPreset)
        {
            ...
            vanillaSettings = TheOtherRolesPlugin.Instance.Config.Bind(string.Format(Helpers.transl("PresetCustomOptions"), preset), "GameOptions", "");
            loadVanillaOptions();
            foreach (CustomOption option in CustomOption.options)
            {
                if (option.id == 0) continue;

                option.entry = TheOtherRolesPlugin.Instance.Config.Bind(string.Format(Helpers.transl("PresetCustomOptions"), preset), option.id.ToString(), option.defaultSelection);
    ...
    class GameOptionsMenuStartPatch
    {
        public static void Postfix(GameOptionsMenu __instance)
        {
            ...
            copyButtonRenderer.sprite = Helpers.loadSpriteFromResources(Helpers.transl("CopyButtonPNG"), 175f);
            ...
            pasteButtonRenderer.sprite = Helpers.loadSpriteFromResources(Helpers.transl("PasteButtonPNG"), 175f);
        ...
        private static void createPropHuntTabs(GameOptionsMenu __instance)
        {
            bool isReturn = setNames(
                new Dictionary<string, string>()
                {
                    ["TORSettings"] = Helpers.transl("PorpHuntSettings")
                });
    ...
    class GameOptionsDataPatch
    {
        ...
        private static string buildModifierExtras(CustomOption customOption)
        {
            ...
            {
                return $" (1 {Helpers.transl("CustomOptionsEvil")}: {CustomOptionHolder.modifierLoverImpLoverRate.getSelection() * 10}%)";
            }
            return "";
        }
            ...
            foreach (var option in options)
            {
                ...
                else if (option.parent.getSelection() > 0)
                {
                    if (option.id == 103) //Deputy
                        sb.AppendLine($"- {Helpers.cs(Deputy.color, Helpers.transl("Deputy"))}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 224) //Sidekick
                        sb.AppendLine($"- {Helpers.cs(Sidekick.color, Helpers.transl("Sidekick"))}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 358) //Prosecutor
                        sb.AppendLine($"- {Helpers.cs(Lawyer.color, Helpers.transl("Prosecutor"))}: {option.selections[option.selection].ToString()}");
                }
            }
