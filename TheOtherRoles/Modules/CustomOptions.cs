...
namespace TheOtherRoles
{
    public class CustomOption
    {
        ...
        public CustomOption(int id, CustomOptionType type, TranslationInfo translationInfo, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, Action onChange = null, TranslationInfo heading = null)
        {
            ...
            if (id != 0)
            {
                entry = TheOtherRolesPlugin.Instance.Config.Bind($"{ModTranslation.GetString("Opt-General", 11)}{preset}", id.ToString(), defaultSelection);
        ...
        {
            return new CustomOption(id, type, translationInfo, new TranslationInfo[] { new TranslationInfo("Opt-General", 9), new TranslationInfo("Opt-General", 10) }, defaultValue ? new TranslationInfo("Opt-General", 10) : new TranslationInfo("Opt-General", 9), parent, isHeader, onChange, heading);
        }
        ...
        public static void switchPreset(int newPreset)
        {
            ...
            vanillaSettings = TheOtherRolesPlugin.Instance.Config.Bind($"{ModTranslation.GetString("Opt-General", 11)}{preset}", "GameOptions", "");
            loadVanillaOptions();
            foreach (CustomOption option in CustomOption.options)
            {
                if (option.id == 0) continue;

                option.entry = TheOtherRolesPlugin.Instance.Config.Bind($"{ModTranslation.GetString("Opt-General", 11)}{preset}", option.id.ToString(), option.defaultSelection);
        ...
        public static int pasteFromClipboard()
        {
            ...
            try
            {
                ...
                ShareOptionSelections();
                if (TheOtherRolesPlugin.Version > versionInfo && versionInfo < Version.Parse("5.9.0"))
                {
                    vanillaOptionsFine = false;
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, ModTranslation.GetString("Opt-General", 28));
                }
                else
                {
            ...
            catch (Exception e)
            {
                ...
                FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, string.Format(ModTranslation.GetString("Opt-General", 86), errorStr));
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType)
        {
            ...
            if (MapOptionsTor.gameMode == CustomGamemodes.Guesser) // Exclude guesser options in neutral mode
            ...
            foreach (var option in relevantOptions)
            {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type)
                {
                    ...
                    categoryHeaderMasked.Title.text = option.heading != null ? option.heading.GetString() : $"<b>{option.getTitle()}</b>";
                    if ((int)optionType == 99)
                ...
                viewSettingsInfoPanel.titleText.text = $"<b>{option.getTitle()}</b>";
                ...
                {
                    viewSettingsInfoPanel.titleText.text = ModTranslation.GetString("Opt-General", 87);
                }
                if ((int)optionType == 99)
                {
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance)
        {
            ...
            int next = 3;
            if (MapOptionsTor.gameMode == CustomGamemodes.Guesser || MapOptionsTor.gameMode == CustomGamemodes.Classic)
            {

                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", ModTranslation.GetString("Opt-General", 89), CustomOptionType.General);
                // create TOR settings
                createCustomButton(__instance, next++, "RoleOverview", ModTranslation.GetString("Opt-General", 88), (CustomOptionType)99);
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", ModTranslation.GetString("Opt-General", 15), CustomOptionType.Impostor);

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", ModTranslation.GetString("Opt-General", 14), CustomOptionType.Neutral);
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", ModTranslation.GetString("Opt-General", 13), CustomOptionType.Crewmate);
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", ModTranslation.GetString("Opt-General", 16), CustomOptionType.Modifier);

            }
            else if (MapOptionsTor.gameMode == CustomGamemodes.HideNSeek)
            {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", ModTranslation.GetString("Opt-General", 90), CustomOptionType.HideNSeekMain);
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", ModTranslation.GetString("Opt-General", 91), CustomOptionType.HideNSeekRoles);
            }
            else if (MapOptionsTor.gameMode == CustomGamemodes.PropHunt)
            {
                createCustomButton(__instance, next++, "PropHunt", ModTranslation.GetString("Opt-General", 92), CustomOptionType.PropHunt);
            }
        }
    }
    ...
    class GameOptionsMenuStartPatch
    {
        ...
        private static void createSettings(GameOptionsMenu menu, List<CustomOption> options)
        {
            ...
            foreach (CustomOption option in options)
            {
                ...
                stringOption.TitleText.text = $"<b>{option.getTitle()}</b>";
                ...
                {
                    stringOption.TitleText.text = ModTranslation.GetString("Opt-General", 87);
                }
        ...
        private static void createSettingTabs(GameSettingMenu __instance)
        {
            ...
            if (MapOptionsTor.gameMode == CustomGamemodes.Guesser || MapOptionsTor.gameMode == CustomGamemodes.Classic)
            {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", ModTranslation.GetString("Opt-General", 89));
                ...
                {
                    createCustomButton(__instance, next++, "GuesserSettings", ModTranslation.GetString("Opt-General", 93));
                    createGameOptionsMenu(__instance, CustomOptionType.Guesser, "GuesserSettings");
                }
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", ModTranslation.GetString("Opt-General", 15));
                createGameOptionsMenu(__instance, CustomOptionType.Impostor, "ImpostorSettings");

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", ModTranslation.GetString("Opt-General", 14));
                createGameOptionsMenu(__instance, CustomOptionType.Neutral, "NeutralSettings");
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", ModTranslation.GetString("Opt-General", 13));
                createGameOptionsMenu(__instance, CustomOptionType.Crewmate, "CrewmateSettings");
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", ModTranslation.GetString("Opt-General", 16));
            ...
            {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", ModTranslation.GetString("Opt-General", 90));
                createGameOptionsMenu(__instance, CustomOptionType.HideNSeekMain, "HideNSeekMain");
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", ModTranslation.GetString("Opt-General", 91));
                createGameOptionsMenu(__instance, CustomOptionType.HideNSeekRoles, "HideNSeekRoles");
            }
            else if (MapOptionsTor.gameMode == CustomGamemodes.PropHunt)
            {
                createCustomButton(__instance, next++, "PropHunt", ModTranslation.GetString("Opt-General", 92));
    ...
    class GameOptionsDataPatch
    {
        ...
        public static string buildModifierExtras(CustomOption customOption)
        {
            ...
            {
                return $" (1 {ModTranslation.GetString("Opt-General", 12)}: {CustomOptionHolder.modifierLoverImpLoverRate.getSelection() * 10}%)";
            }
            return "";
        }
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly)
        {
            ...
            foreach (var option in options)
            {
                ...
                {
                    string line = $"{($"<b>{option.getTitle()}</b>")}: {option.selections[option.selection].ToString()}";
                ...
                {
                    if (option.id == 103) //Deputy
                        sb.AppendLine($"- {Helpers.cs(Deputy.color, ModTranslation.GetString("Role-Name", 11))}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 224) //Sidekick
                        sb.AppendLine($"- {Helpers.cs(Sidekick.color, ModTranslation.GetString("Role-Name", 41))}: {option.selections[option.selection].ToString()}");
                    else if (option.id == 358) //Prosecutor
                        sb.AppendLine($"- {Helpers.cs(Lawyer.color, ModTranslation.GetString("Role-Name", 3))}: {option.selections[option.selection].ToString()}");
                }
            }
            ...
            foreach (CustomOption option in options)
            {
                ...
                if (MapOptionsTor.gameMode == CustomGamemodes.PropHunt && option.type != CustomOptionType.PropHunt) continue;
                if (option.parent != null)
                {
                    ...
                    sb.AppendLine(Helpers.cs(c, $"{($"<b>{option.getTitle()}</b>")}: {option.selections[option.selection].ToString()}"));
                }
                else
                {
                    if (option == CustomOptionHolder.crewmateRolesCountMin)
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Role-Name", 13));
                        ...
                        if (CustomOptionHolder.crewmateRolesFill.getBool())
                        {
                            ...
                            if (min < 0) min = 0;
                            if (max < 0) max = 0;
                            optionValue = $"{ModTranslation.GetString("Role-Name", 17)} ";
                        }
                        if (min > max) min = max;
                    ...
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Role-Name", 14));
                    ...
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Role-Name", 15));
                    ...
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Role-Name", 16));
                    ...
                    {
                        sb.AppendLine($"\n{($"<b>{option.getTitle()}</b>")}: {option.selections[option.selection].ToString()}");
                    }
                }
            }
            return sb.ToString();
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false)
        {
            ...
            else
            {
                ...
                {
                    case 0:
                        hudString += (!hideExtras ? "" : $"{ModTranslation.GetString("Opt-General", 97)} \n\n") + vanillaSettings;
                        break;
                    case 1:
                        hudString += $"{ModTranslation.GetString("Opt-General", 98)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.General, false);
                        break;
                    case 2:
                        hudString += $"{ModTranslation.GetString("Opt-General", 99)} \n" + buildRoleOptions();
                        break;
                    case 3:
                        hudString += $"{ModTranslation.GetString("Opt-General", 100)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Impostor, false);
                        break;
                    case 4:
                        hudString += $"{ModTranslation.GetString("Opt-General", 101)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Neutral, false);
                        break;
                    case 5:
                        hudString += $"{ModTranslation.GetString("Opt-General", 102)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Crewmate, false);
                        break;
                    case 6:
                        hudString += $"{ModTranslation.GetString("Opt-General", 103)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Modifier, false);
                        break;
                }
            }

            if (!hideExtras || counter != 0) hudString += $"\n {ModTranslation.GetString("Opt-General", 104)} ({counter + 1}/{maxPage})";
            return hudString;
        }
    ...
    public class AddToKillDistanceSetting
    {
        ...
        public static bool Prefix(ref string __result, ref StringNames id)
        {
            if ((int)id == 49999)
            {
                __result = ModTranslation.GetString("Opt-General", 29);
                return false;
            }
            return true;
        }
        ...
        public static void addKillDistance()
        {
            ...
            GameOptionsData.KillDistanceStrings = new(new string[] { ModTranslation.GetString("Opt-General", 29), ModTranslation.GetString("Opt-General", 30), ModTranslation.GetString("Opt-General", 31), ModTranslation.GetString("Opt-General", 32) });
        }
    }
